...
- PROPOSE - В статье предлагается представить геометрическую трехмерную модель как распределение вероятностей двоичных переменных на трехмерной воксельной сетке, используя - Convolutional Deep Belief Network. Модель поддерживает распознование образов по карте глубины 2.5D и просматривать планирование распознования объектов.
